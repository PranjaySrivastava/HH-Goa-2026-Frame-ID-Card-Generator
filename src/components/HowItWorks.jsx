...
export default function HowItWorks() {
  return (
    <>
      ...
      <div className="bg-sea-mid pb-16">
        <div className="max-w-[1180px] mx-auto px-5 grid grid-cols-1 md:grid-cols-3 gap-4">
          {STEPS.map((s) => (
            <div key={s.cmd} className="bg-sea-deep/50 border border-sea-line rounded-2xl p-[22px]">
              <div className="font-mono text-[12.5px] text-coral mb-2.5">{s.cmd}</div>
              <h3 className="font-display text-lg mb-2">{s.title}</h3>
              <p className="text-[13.5px] text-sand-dim leading-relaxed m-0">{s.body}</p>
            </div>
          ))}
        </div>
      </div>
